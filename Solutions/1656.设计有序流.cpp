...
private:
    int ptr;
    vector<string> orederStringS;

public:
    OrderedStream(int n)
    {
        ptr = 1;
        orederStringS.resize(n + 1, "");
    }

    vector<string> insert(int id, string value)
    {
        orederStringS[id] = value;
        vector<string> res;
        while (ptr < orederStringS.size() && orederStringS[ptr] != "")
        {
            res.push_back(orederStringS[ptr]);
            ptr++;
        }
        return res;
    }
